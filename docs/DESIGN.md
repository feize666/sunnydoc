...
#### 整图主题（`THEMES`）同样需要亮/暗两份填充

7 套一键换色主题各带 `nodeFill` + `nodeFillDark`（如「默认蓝」`#dbeafe` → `#172554`）。
描边与连线色在两套主题下都已达标（vs 暗底 3.96~7.76），故 `nodeStroke` / `edgeColor` 只存一份。

#### 思维导图：实底 + 白字，必须用 `-solid` token

思维导图节点是**实底 + 白字**（不同于流程图的浅底 + 深字），所以它踩的是 §6.3 那个**对称陷阱**：
...
#### 思维导图分支调色板：104 色中 73 色白字不达标（含亮色主题）

分支节点色板是「一色对应一节点」的实底 + 白字，**在亮色主题下同样要达标**
（不是只有暗色才需要）。审计 104 个色值，**73 个白字 < 4.5**，最低仅 **2.77**（`#0ea5e9`）：

| 修复前 | 白字 | 处置 |
|---|---|---|
| `#0ea5e9`（天蓝） | **2.77** ❌ | 压深 |
| `#0d9488`（青绿） | 3.74 ❌ | 压深 |
| `#0284c7`（海洋） | 4.10 ❌ | 压深至 `#0369a1` → 5.93 |
| `#4c6ef5`（靛蓝/蓝灰） | 4.32 ❌ | 压深至 `#3b5bdb` |
| `#0c8599`（天青） | 4.35  | 压深至 `#0e7490` |
| `#2f6bff`（默认首色） | 4.50（压线） | 压深至 `#1d4ed8` → 6.70 |

压深后主调色板全部 **≥5.02**（`#1d4ed8` 6.70 / `#15803d` 5.02 / `#c2410c` 5.18 /
`#7e22ce` 6.98 / `#0369a1` 5.93 / `#be185d` 6.04 / `#b91c1c` 6.47 / `#57534e` 7.63）。
**只动临界色，其余保持；色相不变** —— 用户认的是「这是绿色那支」，不能改成另一种绿。

> **通用规则**：任何 `-solid` 实底场景（含图表节点，不管在 JS 还是 CSS 里），
> **先算白字对比度再落值**。渲染在文档里的颜色不享受 CSS 审计的覆盖，最容易漏。
